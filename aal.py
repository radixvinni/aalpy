...
#'1' если нужна следующая строка, например если было "def foo():", либо ошибку.
#/output выводит результат предыдущего /execut-а
#нормальная реализация не возможна, поскольку текущая версия jqconsole не поддерживает 
#смену prompt label(ps1, ps2) без сброса консоли. Для этого можно использовать глюканутый jquery-console

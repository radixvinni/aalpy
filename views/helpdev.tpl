<div class="container">
    ...
    <div class="row sidebar" id="pythonhelp">
        ...
        <ul class="nav nav-list well">
            ...
            <li class="">
              <a href="#friends">Дружественные операторы</a>
            </li>
        </ul>
    </div>
...
%%typemap(out) AAL::DecVector {
	...
	for(int i = 0; i < vec.size(); i++) {
      ...
      <div class='section'>
        <div class='page-header'>
          <h2 id='friends'>Дружественные функции и операторы</h2>
        </div>
        <div class='section'>
          <p>
          В языке С++ имеется возможность описывать функции, которые не будут являеться членами класса, но будут иметь доступ к защищенной (protected и private) части класса:
          </p>
          <div class='section'>
            <pre>
class Foo
...
protected:  int prot;
public: 
  friend void Bar(const Foo& foo);  // функция Bar имеет доступ к prot и priv
  friend Foo operator+ (const Foo& a, const Foo& b);
};
            </pre>
          </div>
          <p>
          Всё было бы отлично, но функция-друг не является членом класса. Поскольку python - динамически типизируемый язык, интерпретатор не может ассоциировать функцию-друга с данным классом по типу передаваемого аргумента (но по названию она доступна). В случае же с оператором-другом (например <code>operator+(Foo& a, Foo& b)</code>), поскольку у оператора и имени нет, python вообще не может ассоциировать с ним вызов <code>a+b</code> и такие операторы просто игнорируются. Выход состоит в том, чтобы стараться избегать определения функций и особенно операторов как друзей класса вовсе или объявлять целый класс другом другого класса.
          </p>

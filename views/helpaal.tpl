...
red s(0) + s(s(0)) .
''')</code></pre>
      <p>В данном прмере определяется модуль <code>NAT</code> для представления натуральных чисел:</p>
<pre><code>0 = 0
1 = s(0)
2 = s(s(0))
3 = s(s(s(0)))</code></pre>
      <p>Также определяется операция <code>+</code> и её свойства. Затем предлагается упростить выражение <code>s(0) + s(s(0))</code>,
      что равносильно 1 + 2, оно по-умолчанию выполняется в последнем определённом модуле. Выводится результат 3. 
      Обратите внимание на пробелы между оператором и операндами, они обязательны.</p>
...
red 0 .
red 1 .</code></pre>
      <p>Несколько последовательных вызовов функции <code>maude</code> выполняются на одном интерпретаторе и равносильны
      одному вызову, то есть объявленное в одном вызове можно использовать в последующих. 
      Чтобы сбросить состояние интерпретатора используется функция <code>reset_maude</code>.
      Чтобы номера строк в коде и сообщениях интерпретатора соответствовали, необходимо сбрасывать интерпретатор перед вызовом: 
      <code>reset_maude(); print maude('''...''')</code> и не передавать пустых строк.</p>
      <p>Для выполнения программ maude, которые считаются долго, предусмотрены 4 функции для отдельного вызова интерпретатора:</p>
      <table class="table">
        <tr>
          <th>Функция</th>
          <th>Описание</th>
        ...
        <tr>
          <td><code>start_maude(taskid)</code> <code>start_maude('...')</code></td>
          <td>Вызывает интерпретатор maude с переданным через аргумент кодом подобно функции <code>maude</code>, 
          но на печать выводится только номер задания (taskid), а не вывод интерпретатора.
          Повторный вызов функции с тем же кодом, если выполнение завершилось, напечатает результат выполнения. 
          Вместо строки с кодом второй раз можно передать taskid.</td>
        </tr>
        <tr>
          <td><code>list_maude()</code></td>
          <td>Выводит таблицу всех заданий, выполняющихся и завершённых.</td>
        </tr>
        <tr>
          <td><code>stop_maude(taskid)</code> <code>stop_maude('all')</code></td>
          <td>Останавливает выполнение задания по номеру taskid или всех заданий, если передано <code>'all'</code>.</td>
        </tr>
        <tr>
          <td><code>remove_maude(taskid)</code> <code>remove_maude('all')</code></td>
          <td>Удаляет завершённое задание или останавливает и удаляет все задания, если передано <code>'all'</code>.</td>
        </tr>
      </table>

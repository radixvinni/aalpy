...
2 = s(s(0))
3 = s(s(s(0)))</code>
      <p>Также определяется операция + и её свойства. Затем предлагается упростить выражение <code>s(0) + s(s(0))</code>,
      что равносильно 1 + 2, оно по-умолчанию выполняется в последнем определённом модуле. Выводится результат 3. 
      Обратите внимание на пробелы между оператором и операндом, они обязательны.</p>
...
red 1 .</code></pre>
      ...
      одному вызову, то есть объявленное в одном вызове можно использовать во втором. 
      ...
      <p>Для выполнения программ maude, которые считаются долго, предусмотрены 4 функции отдельного вызова интерпретатора:</p>
      <table class="table">
        <tr>
          <th>Функция</th>
          <th>Описание</th>
        ...
        <tr>
          <td>start_maude(taskid или code)</td>
          ...
          Повторный вызов функции с тем же аргументом если выполнение завершилось, напечатает результат выполнения. 
